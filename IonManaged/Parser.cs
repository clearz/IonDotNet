namespace IonLangManaged
{
    ...
    public unsafe partial class Ion {
        ...
        internal Decls parse_decls() {
            var buf = new List<Decl>();
            while (!is_token(TOKEN_EOF)) {
                buf.Add(parse_decl());
            }

            return new_decls(buf.ToArray(), buf.Count);
        }

        void parse_decls(in IList<Decl> buf) {
            while (!is_token(TOKEN_EOF)) {
                buf.Add(parse_decl());
            }
        }
        ...
        Typespec parse_type_func() {
            ...
            return new_typespec_func(pos, buf.ToArray(), buf.Count, ret, has_varargs);

        }

        Typespec parse_type_tuple() {
            SrcPos pos = token.pos;
            var fields  = new List<Typespec>();
            ...
            return new_typespec_tuple(pos, fields.ToArray(), fields.Count);
        ...
        Typespec parse_type_base() {
            ...
            if (is_token(TOKEN_NAME)) {
                ...
                return new_typespec_name(pos, buf.ToArray(), buf.Count);
            }
        ...
        Expr parse_expr_compound(Typespec type) {
            ...
            var buf = new List<CompoundField>();
            ...
            expect_token(TOKEN_RBRACE);
            return new_expr_compound(pos, type, buf.ToArray(), buf.Count);
        }
        ...
        Expr parse_expr_operand() {
            ...
            if (is_token(TOKEN_FLOAT)) {
                ...
                next_token();
                return new_expr_float(pos, new string(start, 0, (int)(end - start)), val, suffix);
            }
            ...
            if (is_token(TOKEN_NAME)) {
                ...
                if (is_token(TOKEN_LBRACE))
                    return parse_expr_compound(new_typespec_name(pos, new string[] { name }, 1));
        ...
        Expr parse_expr_base() {
            ...
            while (is_token(TOKEN_LPAREN) || is_token(TOKEN_LBRACKET) || is_token(TOKEN_DOT)
                ...
                if (match_token(TOKEN_LPAREN)) {
                    ...
                    expect_token(TOKEN_RPAREN);
                    expr = new_expr_call(pos, expr, buf.ToArray(), buf.Count);
                }
        ...
        StmtList parse_stmt_block() {
            ...
            var buf = new List<Stmt>();
            while (!is_token_eof() && !is_token(TOKEN_RBRACE))
                buf.Add(parse_stmt());

            expect_token(TOKEN_RBRACE);
            return stmt_list(pos, buf.ToArray(), buf.Count);
        ...
        Stmt parse_stmt_if(SrcPos pos) {
            ...
            return new_stmt_if(pos, init, cond, then_block, buf.ToArray(), buf.Count, else_block);
        ...
        SwitchCase parse_stmt_switch_case() {
            var patterns = new List<SwitchCasePattern>();
        ...
        Stmt parse_stmt_switch(SrcPos pos) {
            ...
            return new_stmt_switch(pos, expr, buf.ToArray(), buf.Count);
        }
        ...
        Decl parse_decl_enum(SrcPos pos) {
            ...
            expect_token(TOKEN_RBRACE);
            return new_decl_enum(pos, name, type, items.ToArray(), items.Count);
        }
        ...
        Aggregate parse_aggregate(AggregateKind kind) {
            ...
            expect_token(TOKEN_RBRACE);
            return new_aggregate(pos, kind, items.ToArray(), items.Count);
        }
        ...
        Decl parse_decl_func(SrcPos pos) {
            ...
            var decl = new_decl_func(pos, name, buf.ToArray(), buf.Count, ret_type, has_varargs, varargs_type, block);
        ...
        Note parse_note() {
            ...
            return new_note(pos, name, args.ToArray(), args.Count);
        }

        Notes parse_notes() {
            var buf = new List<Note>();
            while (match_token(TOKEN_AT)) {
                buf.Add(parse_note());
            }
            return new_notes(buf.ToArray(), buf.Count);
        }
...
repeat:
            ...
            return new_decl_import(pos, rename_name, is_relative, names.ToArray(), names.Count, import_all, items.ToArray(), items.Count);
        }
