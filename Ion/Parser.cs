...
namespace Lang
{
    ...
    public unsafe partial class Ion
    {
		...
		static readonly string[] decls2 = {
			"var i{0}: int = cast(int*, 42)",
			"var x{0}: char[256] = {1, 2, 3, ['a'] = 4}",
			"struct Vector{0} { x, y: float; }",
			"var v{0} = Vector{x = 1.0, y = -1.0}",
			"var v{0}: Vector = {1.0, -1.0}",
			"const n{0} = sizeof(:int*[16])",
			"const n{0} = sizeof(1+2)",
			"var x{0} = b{0} == 1 ? 1+2 : 3-4",
			"func fact{0}(n: int): int { trace(\"fact\"); if (n == 0) { return 1; } else { return n * fact(n-1); } }",
			"func fact{0}(n: int): int { p := 1; for (i := 1; i <= n; i++) { p *= i; } return p; }",
			"var foo{0} = a ? a&b + c<<d + e*f == +u-v-w + *g/h(x,y) + -i%k[x] && m <= n*(p+q)/r : 0",
			"func f{0}(x: int): bool { switch(x) { case 0: case 1: return true; case 2: default: return false; } }",
			"enum Color{0} { RED = 3, GREEN, BLUE = 0 }",
			"const pi{0} = 3.14",
			"union IntOrFloat{0} { i: int; f: float; }",
			"typedef Vectors{0} = Vector[1+2]",
			"func f{0}() { do { print(42); } while(1); }",
			"typedef T = (func(int):int)[16]",
			"func f{0}() { enum E{0} { A, B, C } return; }",
			"func f{0}() { if (1) { return 1; } else if (2) { return 2; } else { return 3; } }",
		};
